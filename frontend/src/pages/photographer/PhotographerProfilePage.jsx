...
import { photographerClient, photographerTokenStore, usePhotographerAuth } from "../../context/PhotographerAuthContext.jsx";
...
export default function PhotographerProfilePage() {
  const { user, applyUser } = usePhotographerAuth();
  ...
  const submit = async (e) => {
    ...
    setError("");
    try {
      const res = await photographerClient.patch("/api/users/me", form);
  ...
  const handleAvatar = async (e) => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await photographerClient.post("/api/users/me/avatar", fd, { headers: { "Content-Type": "multipart/form-data" } });
      applyUser(res.data.user);
  ...
  return (
    <PhotographerConsoleLayout title="Profile" subtitle="Your account details">
      ...
      <div className="stack gap-5">
        ...
        <ChangePasswordCard email={user?.email} client={photographerClient} tokenStore={photographerTokenStore} />
      </div>
